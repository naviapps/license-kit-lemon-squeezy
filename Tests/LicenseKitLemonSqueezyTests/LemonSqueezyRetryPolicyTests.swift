...
final class LemonSqueezyRetryPolicyTests: XCTestCase {
  func testNormalizesNonPositiveConfigurationValues() {
    let policy = LemonSqueezyRetryPolicy(maximumAttempts: 0, baseDelayMilliseconds: 0)
    ...
    XCTAssertFalse(policy.shouldRetry(statusCode: 500, afterAttempt: 1))
  }

  func testShouldRetryHonorsStatusAndAttempt() {
    let policy = LemonSqueezyRetryPolicy(maximumAttempts: 3, baseDelayMilliseconds: 200)
    XCTAssertTrue(policy.shouldRetry(statusCode: 500, afterAttempt: 1))
    XCTAssertTrue(policy.shouldRetry(statusCode: 429, afterAttempt: 2))
    XCTAssertFalse(policy.shouldRetry(statusCode: 400, afterAttempt: 1))
    XCTAssertFalse(policy.shouldRetry(statusCode: 503, afterAttempt: 3))
  }
